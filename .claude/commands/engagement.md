...
1. Run `lolmcp doctor` first — confirm frida, objection, mitmdump, and vendored projects are present.
2. Start the engagement:
   ```
   lolmcp run --target $ARGUMENTS
   ```
...
5. If any high or critical finding is present, suggest a `lolmcp replay <finding_id>` confirmation.

Don't fabricate findings. If `lolmcp run` errors before reaching mapping, debug the cause (device offline, proxy not intercepting, target not launching) and report back honestly.
